...
export async function normalizeRawAssociationsForSubject(
  db: DbClient,
  args: { fileUuid: string; subjectIndividualId: string },
): Promise<void> {
  const treeId = await resolveTreeIdForFileUuid(db, args.fileUuid);
  await ensureDefaultRelationshipTypes(db, treeId);
  const types = await db.relationshipType.findMany({
    where: { OR: [{ treeId }, { treeId: null }] },
    include: { roles: true },
  });
  const typeByKey = new Map(types.map((t) => [t.key, t]));
  const rows = await db.gedcomIndividualAssociation.findMany({
    where: { fileUuid: args.fileUuid, subjectIndividualId: args.subjectIndividualId },
  ...
  for (const row of rows) {
    ...
    if (!type) continue;
    const roleA = type.roles.find((r) => r.key === roleKey) ?? type.roles[0];
    if (!roleA) continue;
    const roleB = type.roles.find((r) => r.key === roleA.reciprocalRoleKey) ?? type.roles[0];
    if (!roleB) continue;
    let relationship = await findEquivalentTwoPartyRelationship(db, {
      fileUuid: args.fileUuid,
      relationshipTypeId: type.id,
      firstIndividualId: row.subjectIndividualId,
      firstRoleId: roleA.id,
      secondIndividualId: row.associateIndividualId,
      secondRoleId: roleB.id,
    });
    if (!relationship) {
      relationship = await db.individualRelationship.create({
        data: {
          fileUuid: args.fileUuid,
          relationshipTypeId: type.id,
          ...
          participants: {
            create: [
              { individualId: row.subjectIndividualId, roleId: roleA.id, sortOrder: 0 },
              { individualId: row.associateIndividualId, roleId: roleB.id, sortOrder: 1 },
            ],
          },
        },
        include: { participants: true },
    ...
    await db.individualRelationshipSourceAssociation.upsert({
      where: {
