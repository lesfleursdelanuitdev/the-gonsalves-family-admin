...
export function buildExportBundleReadme(opts: {
  ...
  const lines: string[] = [
    ...
    "",
    "- manifest.json",
    "    Verifiable backup manifest: UTC time, tree id (Gedcom file UUID), app/schema hints, entity counts,",
    "    and SHA-256 + byte size for README, GEDCOM, JSON, CSV, and each bundled media file.",
    "    (manifest.json is not listed inside its own `files` array; verify it separately if needed.)",
    "",
    "- media/",
