#!/usr/bin/env node
/**
 * Seed any StoryDocument JSON file into the database.
 *
 ...
 * - Prefers the Site Admin role holder as author; falls back to the first user.
 *
 * Usage (from the-gonsalves-family-admin):
 *   node --experimental-strip-types scripts/seed-story.ts --json=data/stories/buxton-portuguese-history.json --dry-run
 *   node --experimental-strip-types scripts/seed-story.ts --json=data/stories/buxton-portuguese-history.json --execute
 *
...
function parseArgs(argv: string[]) {
  ...
  for (const a of argv) {
    ...
    else if (a.startsWith("--json=")) jsonPath = path.resolve(repoRoot, a.slice("--json=".length));
  }
...
async function main() {
  const { dryRun, execute, jsonPath } = parseArgs(process.argv.slice(2));

  if (!jsonPath) {
    console.error("Pass --json=<path> to specify the story JSON file.");
    console.error("Example: node --experimental-strip-types scripts/seed-story.ts --json=data/stories/buxton-portuguese-history.json --dry-run");
    process.exit(1);
  }
  ...
  console.log(`kind=${doc.kind ?? "story"}`);
