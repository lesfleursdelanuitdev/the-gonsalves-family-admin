...
import type { FamilyEditorSectionId } from "@/components/admin/family-editor/family-editor-nav";

export type FamilyEditorAccordionKey = FamilyEditorSectionId;

type Props = {
  id: FamilyEditorSectionId;
  sectionKey: FamilyEditorAccordionKey;
  title: string;
  description: string;
  icon: LucideIcon;
  summary: string;
  isDesktop: boolean;
  mobileExpanded: FamilyEditorAccordionKey | null;
  onMobileToggle: (key: FamilyEditorAccordionKey) => void;
  children: ReactNode;
};

/** Desktop: open section card. Mobile: collapsible accordion (same pattern as the person editor). */
export function FamilyEditorResponsiveSection({
  id,
  sectionKey,
  title,
  description,
