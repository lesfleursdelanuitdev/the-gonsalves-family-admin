...
export const DELETE = withAdminAuth(async (_request, user, ctx) => {
  ...
  const existing = await prisma.userMediaTag.findFirst({
    where: { id: linkId, userMediaId, taggedBy: user.id },
  });
  if (!existing) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await commitMediaJunctionUnlink(changeCtx, "user_media_tag", existing, "Removed user media app tag");
    await tx.userMediaTag.delete({ where: { id: linkId } });
  });
