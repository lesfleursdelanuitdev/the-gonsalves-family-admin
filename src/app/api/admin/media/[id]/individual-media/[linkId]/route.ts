...
export const DELETE = withAdminAuth(async (_req, user, ctx) => {
  ...
  const existing = await prisma.gedcomIndividualMedia.findFirst({
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await commitMediaJunctionUnlink(changeCtx, "individual_media", existing, "Removed individual–media link");
    await tx.gedcomIndividualMedia.delete({ where: { id: linkId } });
  });
