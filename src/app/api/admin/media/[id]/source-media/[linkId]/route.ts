...
export const DELETE = withAdminAuth(async (_req, user, ctx) => {
  ...
  const existing = await prisma.gedcomSourceMedia.findFirst({
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await commitMediaJunctionUnlink(changeCtx, "source_media", existing, "Removed source–media link");
    await tx.gedcomSourceMedia.delete({ where: { id: linkId } });
  });
