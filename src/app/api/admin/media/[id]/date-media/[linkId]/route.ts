...
export const DELETE = withAdminAuth(async (_req, user, ctx) => {
  ...
  const existing = await prisma.gedcomMediaDate.findFirst({
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await commitMediaJunctionUnlink(changeCtx, "media_date", existing, "Removed date–media link");
    await tx.gedcomMediaDate.delete({ where: { id: linkId } });
  });
