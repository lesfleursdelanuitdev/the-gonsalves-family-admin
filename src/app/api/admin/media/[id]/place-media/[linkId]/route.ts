...
export const DELETE = withAdminAuth(async (_req, user, ctx) => {
  ...
  const existing = await prisma.gedcomMediaPlace.findFirst({
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await commitMediaJunctionUnlink(changeCtx, "media_place", existing, "Removed place–media link");
    await tx.gedcomMediaPlace.delete({ where: { id: linkId } });
  });
