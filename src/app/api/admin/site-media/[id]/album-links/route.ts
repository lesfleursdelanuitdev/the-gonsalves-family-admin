...
export const POST = withAdminAuth(async (request, user, ctx) => {
  ...
  try {
    const row = await prisma.$transaction(async (tx) => {
      const media = await tx.siteMedia.findFirst({
      ...
      const created = await tx.albumSiteMedia.create({
        data: { albumId, siteMediaId, addedBy: user.id },
      ...
      await commitMediaJunctionLink(
        ...
        "album_site_media",
        created,
        `Added site media to album “${created.album.name}”`,
      );
      return created;
    });
