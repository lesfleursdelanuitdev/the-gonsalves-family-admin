...
export const DELETE = withAdminAuth(async (_req, user, ctx) => {
  ...
  const { id: familyId, linkId } = await ctx.params;
  const fileUuid = await getAdminFileUuid();

  const existing = await prisma.gedcomFamilySource.findFirst({
    where: { id: linkId, familyId, fileUuid },
  });
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await commitMediaJunctionUnlink(changeCtx, "family_source", existing, "Removed family citation");
    await tx.gedcomFamilySource.delete({ where: { id: linkId } });
  });
