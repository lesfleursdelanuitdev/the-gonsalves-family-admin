...
export const PATCH = withAdminAuth(async (req, _user, ctx) => {
  ...
  const row = await prisma.eventType.findUnique({ where: { id }, select: { isCustom: true, fileUuid: true } });
  if (!row) return NextResponse.json({ error: "Not found" }, { status: 404 });
  if (!row.isCustom || row.fileUuid !== fileUuid) {
    return NextResponse.json({ error: "Standard event types cannot be modified." }, { status: 403 });
  }
...
export const DELETE = withAdminAuth(async (_req, _user, ctx) => {
  ...
  if (!row) return NextResponse.json({ error: "Not found" }, { status: 404 });
  if (!row.isCustom || row.fileUuid !== fileUuid) {
    return NextResponse.json({ error: "Standard event types cannot be deleted." }, { status: 403 });
  }
